...
с помощью методов onSaveInstanceState и onRestoreInstanceState сохраняем и восстанавливаем заниение счётчика. останавливается и запускается счетчик по буллевой переменной, которая изменяется в методах onPause и onResume
